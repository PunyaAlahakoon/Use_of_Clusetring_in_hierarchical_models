...
%BLOCKWISE & COMPONENETWISE sampling 
...
g=find(group==1);%change this according to the cluster you want to study

%load parameter sets of all the populations as matrices
me=beta_smc(:,g); %from Step (1)--a  
ga=gamma_smc(:,g);
ep=epsilon_smc(:,g);
...
r=[0.001 0.00001 0.01]; s=[10 3 .2]; %truncated interval for truncated normal 
pd=@(m) makedist('Normal','mu',m,'sigma',0.0003);
tpd=@(m) truncate(pd(m),r(3),s(3));

w=weight7(me,ga,ep,hb(:,1),hb_sig(:,1),dim,r,s);
   ...
   for j=1:3
       ...
       elseif j==2
        ...
        hh=[hb(1,i-1) hm_r hb(3,i-1)]; ss=[hb_sig(1,i-1) sig_r hb_sig(3,i-1)];  
           ...
           hB=zeros(1,2);
           while  hB(1)<0.01 || hB(1)>0.2
             hB=abs(mvnrnd([hb(j,i-1) hb_sig(j,i-1)],eye(2)*0.0003));
            hm_r=hB(1); sig_r=hB(2);
        %hm_r= random(tpd(hb(j,i-1))); sig_r=abs(normrnd(hb_sig(j,i-1),0.0003));
         hh=[hb(1:2,i-1)' hm_r]; ss=[hb_sig(1:2,i-1)' sig_r];   
           end
       end

   w=weight7(me,ga,ep,hh,ss,dim,r,s);
...
hb_sig(:,i)=x_sig;
  ...
  w=weight7(me,ga,ep,hb(:,i),hb_sig(:,i),dim,r,s);
