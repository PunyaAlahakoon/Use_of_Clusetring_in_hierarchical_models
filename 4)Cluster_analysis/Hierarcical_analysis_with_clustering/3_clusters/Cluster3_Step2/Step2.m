...
tpd=@(f,h) truncate(pd_b(f,h),1,10);
    ...
    hb=load('hb_g3_all.mat', 'hb'); %all the hyper_means ,beta, gamma, epsilon order 
    hb=hb.hb;
    hb=hb(:,5001:10000); 
  
    hb_sig=load('hb_sig_g3_all.mat', 'hb_sig'); %all the hyper_sigams 
...
save('hie_cluster3_beta_g3.mat','beta_smc');
save('hie_cluster3_gamma_g3.mat','gamma_smc');
save('hie_cluster3_epsilon_g3.mat','epsilon_smc');
