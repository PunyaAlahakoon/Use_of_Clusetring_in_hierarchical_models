...
tic 
%load data
data=load('sc3_data.mat', 'data'); % load the dataset you need
...
%load parmeters and weigts:
betas=load('independent_beta_d3.mat');
betas=betas.beta_smc;
gammas=load('independent_gamma_d3.mat');
gammas=gammas.gamma_smc;
epsilons=load('independent_epsilon_d33.mat');
epsilons=epsilons.epsilon_smc;

weights=load('w_smc1.mat');
weights=weights.w_smc;
...
for k=1:dim
    ...
    params=[betas(:,k) gammas(:,k) epsilons(:,k)];
   % betas=zeros(1,B); %store betas
    %gammas=zeros(1,B);
    %epsilons=zeros(1,B);
    w=weights(:,k)'; %store weights 
    %es=zeros(1,G+1);
    %es(1)=e;
...
save('s_x_smc2.mat','s_x');
save('independent_beta_d33.mat','beta_smc');
save('independent_gamma_d33.mat','gamma_smc');
save('independent_epsilon_d33.mat','epsilon_smc');
%save('E_smc1','E_smc');
